...
/*   Updated: 2024/09/11 17:21:33 by emfourni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// void	print_split(char **split, int nb_cmd, int nb_node)
// {
// 	int index;
...
void	cmd_handler(char *cmd_line, t_prompt *prompt)
{
	...
	while (index < nb_pipes)
	{
		...
		// print_split(prompt->cmd, ft_countword(cmd_split[index], ' '), index);
		promptadd_back(&prompt, prompt_init(prompt));
		redirect_handler(cmd_split[index], prompt);
		index++;
	}
	free_cmd(cmd_split, nb_pipes);
}
